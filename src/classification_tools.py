...
Description:
Program that provides tools for evaluating and visualizing logistic regression model performance in Python,
including functions for summary statistics, plotting decision boundaries, and generating performance metrics
like ROC curves and confusion matrices. It supports both standalone models and those within a scikit-learn pipeline.
"""
...
def plot2DClass(X: pd.core.frame.DataFrame, y: pd.core.series.Series, model, var1:str, var2:str, selClass:str=None, np_grid=200):
    ...
    if len(df.columns) == 3:
        ...
        grid_X1_X2.columns = [var1, var2, output_name] + ['_'.join(['prob',lev]) if isinstance(lev, str) else '_'.join(['prob',str(lev)]) for lev in y.unique()]
        # Define output class variable in grid
        grid_X1_X2['prob'] = grid_X1_X2['_'.join(['prob',selClass])]
        # Classification of input space
        ax = plt.subplot(221)
        sns.scatterplot(x=var1, y=var2, hue=output_name, palette=palette, ax=ax, data=grid_X1_X2).set_title('Classification of input space')
        ...
        sns.scatterplot(x=var1, y=var2, hue='prob', data=grid_X1_X2, palette=sns.color_palette("Blues", as_cmap=True)).set_title(' '.join(["Probabilities estimated for input space, class:",selClass]))
        ...
        ax = plt.subplot(224)
        df[output_name] = y
        sns.scatterplot(x=var1, y=var2, hue=output_name, palette=palette, ax=ax, data=df).set_title(' '.join(['classes and estimated probability contour lines for class:', selClass]))
...
def roc_curve(real, estimations, selClass:str, fpr_label='False Positive Rate', tpr_label='True Positive Rate', title='Receiver operating characteristic (ROC)'):
